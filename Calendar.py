...
def command_add(date, start_time, end_time, title, calendar):
    ...
    if start_time > end_time:
        return False
    '''
    if end_time < start_time or not 0 <= start_time <= 23 or not 0 <= end_time <= 23:
        return False
    if date in calendar:
        calendar[date].append({"start": start_time, "end": end_time, "title": title})
        calendar = sort_calendar(calendar)
        return True
    else:
...
def sort_calendar(calendar):
    sorted_calendar = {}
    for key in sorted(calendar, reverse=True):
        sorted_calendar[key] = sorted(calendar[key], key=lambda x : x.get("start"))
...
def command_show(calendar):
    ...
    data = ""
    calendar = sort_calendar(calendar)
    keys_calender = calendar.keys()

    for each in keys_calender:
        i = 0
        data += "\n" + each + " : "
        for j, value in enumerate(calendar.get(each)):
            data += "\n    start : " +datetime.time(hour=value.get("start")).strftime("%H:%M")
            data += ",\n    end : " + datetime.time(hour=value.get("end")).strftime("%H:%M")
            data += ",\n    title : " + value.get("title")
            if j != len(calendar.get(each)) - 1:
                data += "\n"
            i += 1
    return data
...
def command_delete(date, start_time, calendar):
    ...
    def has_key(value):
        ...
        if key is not None:
            ...
            return False
    if has_key(date):
        values = calendar.get(date)
        if len(values) >= 1:
            #we have to compare start_time from function param to object property
            for i in range(len(values)):
                if values[i]['start'] == start_time:
                    values.pop(i)
                    if len(values) == 0:
                        del calendar[date]
                    return True
...
def save_calendar(calendar):
    ...
    try:
        ...
        with open("calendar.txt", 'w') as file:
            for key in calendar:
                file.write(key + ":")
                for i, agenda in enumerate(calendar[key]):
                    start_time = datetime.time(hour=agenda.get("start")).strftime("%H")
                    end_time = datetime.time(hour=agenda.get("end")).strftime("%H")
                    title = agenda.get("title")

                    file.write("%s-%s %s"%(start_time, end_time, title))
                    if i != len(calendar[key]) - 1:
                        file.write("\t")
                    else:
                        file.write("\n")
        return True
            ...
            with open("calendar.txt", "r") as file:
                for line in file:
                    ...
                    for agenda in agendas:
                        ...
                        title = " ".join(bar[1:])
                        calendar[date].append({"start": start, "end": end, "title": title})
    else:
        calendar = {}
    return calendar
...
def parse_command(line):
    ...
    if len(line) == 0:
        return ["help"]
    if line[0] == "add":
        eventDetails = " ".join(line[4:])
        if len(line) < 5:
            return ["error", "add DATE START_TIME END_TIME DETAILS"]
        elif not is_calendar_date(line[1]):
            return ["error", "not a valid calendar date"]
        else:
            return ["add", line[1],  int(line[2]), int(line[3]), eventDetails]
    elif line[0] == "delete":
        if len(line) != 3:
            return ["error", "delete DATE START_TIME"]
        elif not is_calendar_date(line[1]):
            return ["error", "not a valid calendar date"]
        elif not is_natural_number(line[2]):
            return ["error", "not a valid event start time"]
        else:
            return ["delete", line[1], int(line[2])]
    elif line[0] == "show":
        if len(line) != 1:
            return ["error", "show"]
        else:
